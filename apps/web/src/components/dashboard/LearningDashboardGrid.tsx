...
import {
  computeRetentionBars,
  computeScoreDelta,
  computeSessionsLast7Days,
  computeStudyStreak,
  computeSubjectShares,
...
function Sessions() {
	const { explanations, totalSessionsToday } = useNotes()
	const counts = computeSessionsLast7Days(explanations)
	const peak = Math.max(...counts, 1)
	const dayLabels = counts.map((_, index) => {
		const date = new Date()
	...
	return (
		<Shell title="Study sessions" meta="last 7 days">
			<Big>{fmt(totalSessionsToday)}</Big>
			<div className="mt-auto">
				<div
					role="img"
					aria-label={`${fmt(totalSessionsToday)} Feynman sessions completed today.`}
					className="flex h-10 items-end gap-[3px]">
					{counts.map((count, index) => (
						<span
							key={dayLabels[index]}
							title={`${dayLabels[index]}: ${count} sessions`}
							className={`flex-1 rounded-full ${index === counts.length - 1 ? ACCENT : 'bg-foreground/15'}`}
...
function Retention() {
	const { retentionAverage, notes } = useNotes()
	const bars = computeRetentionBars(notes)
	const degraded = retentionAverage < 85
	return (
		<Shell title="Retention health" meta="portfolio">
			...
			<div className="mt-auto">
				<div
					role="img"
					aria-label="Retention health across your notes"
					className="flex h-5 gap-[2px] @[240px]:h-6">
					{bars.map((value, i) => (
						<span
							key={i}
							className={`flex-1 rounded-[1.5px] ${
								value >= 85 ? 'bg-emerald-400/80' : value >= 70 ? 'bg-gold/70' : 'bg-amber-400/80'
							}`}
...
function Streak() {
	const { explanations } = useNotes()
	const streak = computeStudyStreak(explanations)
	const counts = computeSessionsLast7Days(explanations)
	const max = Math.max(...counts, 1)

	return (
		<Shell title="Study streak" meta="consecutive days">
			<Big unit="days">{streak}</Big>
			<div
				role="img"
				aria-label={`Study streak of ${streak} days.`}
				className="mt-auto flex h-10 items-end gap-[3px]">
				{counts.map((count, i) => (
					<span
						key={i}
						className={`flex-1 rounded-full ${
							i === counts.length - 1 ? ACCENT : 'bg-foreground/15'
						}`}
...
function LectorScore() {
	...
	const delta = computeScoreDelta(explanations)
	const correctness = latestExp ? (latestExp.correctness / 100).toFixed(2) : '—'
	const clarity = latestExp ? (latestExp.clarity / 100).toFixed(2) : '—'
	const completeness = latestExp ? (latestExp.completeness / 100).toFixed(2) : '—'
	...
	return (
		<Shell title="LECTOR score">
			<div className="flex flex-wrap items-baseline gap-x-2.5 gap-y-1">
				...
				{latestExp && (
					<span className={`text-[14px] tabular-nums ${delta >= 0 ? TEXT.ok : TEXT.err}`}>
...
function DueRevisions() {
	const { notes, studyMode } = useNotes()
							...
							<div className="flex items-center gap-2 shrink-0">
								<span
									...
									{isDueNow
										? 'Due Today'
										: n.interval
											? formatReviewIntervalLabel(n.interval, studyMode)
											: n.nextReviewDate}
								</span>
...
const SWATCHES = [ACCENT, 'bg-gold/55', 'bg-gold/35', 'bg-foreground/20']

function Subjects() {
	const { notes } = useNotes()
	const subjects = computeSubjectShares(notes)

	return (
		<Shell title="Topics by subject" meta="all notes">
			<Big unit="concepts">{notes.length}</Big>
			{subjects.length === 0 ? (
				<p className="mt-auto text-[13px] text-muted-foreground">Add notes to see subject breakdown.</p>
			) : (
				<>
					<dl className="mt-auto grid grid-cols-2 gap-x-6 gap-y-2">
						{subjects.slice(0, 4).map((subject, index) => (
							<Row key={subject.name} value={`${Math.round(subject.share * 100)}%`}>
								<span
									aria-hidden="true"
									className={`size-1.5 shrink-0 rounded-full ${SWATCHES[index] ?? SWATCHES[3]}`}
								/>
					...
					<div
						role="img"
						aria-label={`Share of concepts by subject.`}
						className="mt-4 flex h-[3px] gap-[3px]">
						{subjects.slice(0, 4).map((subject, index) => (
							<span
								key={subject.name}
								className={`h-full rounded-full ${SWATCHES[index] ?? SWATCHES[3]}`}
								style={{ width: `${subject.share * 100}%` }}
							/>
						))}
					</div>
				</>
