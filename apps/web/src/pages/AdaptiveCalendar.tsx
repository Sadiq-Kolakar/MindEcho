...
import { formatReviewIntervalLabel } from '../lib/dashboard-stats'
...
export function AdaptiveCalendar() {
  ...
  const avgInterval =
    notes.filter((n) => n.interval && n.interval > 0).reduce((sum, n) => sum + (n.interval ?? 0), 0) /
      Math.max(1, notes.filter((n) => n.interval && n.interval > 0).length) || 1
  const revisionLabel = formatReviewIntervalLabel(Math.round(avgInterval), studyMode)
          ...
          <GlassCard dark className="p-5 border border-white/15 shadow-xl">
            <div className="flex items-center justify-between">
              ...
              <span className="rounded-full bg-blue-500/20 px-2.5 py-0.5 text-xs font-bold text-blue-300">
                {studyMode === 'exam' ? 'Exam compression active' : 'Skill mode'}
              </span>
            </div>
            <p className="mt-2 text-2xl font-bold text-white">{revisionLabel}</p>
            <p className="mt-1 text-[11px] text-white/60">
              {studyMode === 'exam'
                ? 'Intervals compress toward exam and important dates'
                : 'Intervals expand based on your LECTOR scores'}
            </p>
          </GlassCard>
        </div>
                  ...
                  dueNotesOnSelectedDate.map((note) => (
                    ...
                    >
                      ...
                      <div className="mb-3 text-[11px] text-white/60 flex items-center justify-between">
                        ...
                        <span>
                          {note.interval
                            ? formatReviewIntervalLabel(note.interval, studyMode)
